...
class CustomPyTorchDeepIDExplainer(Explainer):

    def __init__(self, model, data, masks, gpu_memory_efficient=False):
        ...
        self.num_outputs = 1
        with torch.no_grad():
            outputs = self.get_output(data, masks)
            # print(outputs.shape)
            ...
            for j in range(X[0].shape[0]):
                ...
                if self.interim:
                    ...
                    for l in range(len(X)):
                        phis[l][j] = (torch.from_numpy(sample_phis[l][self.data[l].shape[0]:]) * (X[l][j: j + 1] - self.data[l]).cpu()).mean(0)
