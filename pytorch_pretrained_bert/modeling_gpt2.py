...
class GPT2PreTrainedModel(nn.Module):
    ...
    def from_pretrained(
        ...
        model.set_num_special_tokens(num_special_tokens if num_special_tokens is not None else 0)
        model.config.num_special_tokens = num_special_tokens
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def set_num_special_tokens(self, num_special_tokens):
        ...
        self.total_tokens = self.config.vocab_size + num_special_tokens
        # # Build new embeddings and initialize
        old_embed = self.wte
        self.wte = nn.Embedding(self.total_tokens, self.config.n_embd)
        ...
        self.wpe.weight.data[-self.config.n_positions :, :] = old_embed.weight.data[-self.config.n_positions :, :]
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.transformer = GPT2Model(config, num_special_tokens)
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.lm_head = GPT2LMHead(self.transformer.wte.weight, config)
        # this is a hack, change it later, we assume the clf_token is the last token in vocab here
        clf_token = self.transformer.wte.weight.size()[0]
        self.task_head = ClfHead(clf_token, config, 2) 
    ...
    def set_num_special_tokens(self, num_special_tokens):
        """ Update input and output embeddings with new embedding matrice
            Make sure we are sharing the embeddings
        """
        self.config.total_tokens = self.config.vocab_size + num_special_tokens
        self.transformer.set_num_special_tokens(num_special_tokens)
        self.set_tied()
    ...
    def forward(self, input_ids, mc_token_ids, lm_labels=None, mc_labels=None, token_type_ids=None, position_ids=None, past=None):
        ...
        mc_logits = self.task_head(hidden_states, input_ids)
        
        losses = []
        if lm_labels is not None:
            shift_logits = lm_logits[:,0, :-1].contiguous()
            shift_labels = lm_labels[:,0, 1:].contiguous()
            loss_fct = CrossEntropyLoss(ignore_index=-1)
