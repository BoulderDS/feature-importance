...
class GPT2Tokenizer(object):
    ...
    def __init__(self, vocab_file, merges_file, errors='replace', max_len=None, special_tokens=None):
    ...
    def encode(self, text):
        ...
        for token in re.findall(self.pat, text):
            before_encode_len = len(token)
            token = ''.join(self.byte_encoder[b] for b in token.encode('utf-8'))
            
            bpe_token_ls = [bpe_token for bpe_token in self.bpe(token).split(' ')]
            bpe_tokens.extend(self.encoder[bpe_token] for bpe_token in bpe_token_ls)
            token_spans = []
            for bpe_token in bpe_token_ls:
                token_spans.append((position, len(bpe_token) + position))
                position += len(bpe_token)
